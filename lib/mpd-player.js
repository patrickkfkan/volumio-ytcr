...
class MPDPlayer extends Player {
    ...
    async loadPlayData(videoId) {
        ...
        if (data.playable) {
            ...
            data.streamUrl = highestAudioFormat.url.replace(/"/g, '\\"'); // safe uri
            data.channels = highestAudioFormat.audioChannels;
            data.bitrate = highestAudioFormat.audioBitrate;
        }
